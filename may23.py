...
def load_sample_stream():
    return obspy.read(
        "c:/Users/lworthington/Downloads/8J.009.001.991.sgy", unpack_trace_headers=True
    )
...
def plot_stream(st, show=True):
    ...
    for i in range(len(st)):
        offset = (
            st[
                i
            ].stats.segy.trace_header.distance_from_center_of_the_source_point_to_the_center_of_the_receiver_group
            / 1000
        )
...
def fix_offsets(old_st):
    ...
    for i in range(len(old_st)):
        try:
            tr = old_st[i].copy()
            tr.stats.segy.trace_header.distance_from_center_of_the_source_point_to_the_center_of_the_receiver_group = source_receiver_offset(
                tr
            )
            st += tr
        except:
