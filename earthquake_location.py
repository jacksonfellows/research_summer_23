...
def demo_inversion(source, stations, arrivals, v, N):
    source_guess = np.random.normal(0, 0.01, 3) + np.concatenate((stations[arrivals.argmin()], np.array([arrivals.min()])))
...
def demo_with_random_data():
    plt.xlim(0, 10)
    plt.ylim(0, 10)
    source = np.concatenate((10 * np.random.random(2), np.zeros(1)))  # earthquake source (x,y,t)
    stations = 10 * np.random.random((7, 2)) # station locations
    v = 1
    arrivals = np.random.normal(0, 0.1, stations.shape[0]) + simulate_arrivals(source, stations, v)
